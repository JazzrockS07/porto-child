...
if ( ( function_exists( 'woocommerce_product_loop' ) && woocommerce_product_loop() ) || ( ! function_exists( 'woocommerce_product_loop' ) && have_posts() ) ) {
	...
	if ( ! $builder_id ) {
		if (is_product_category('birthday-gifts')) {
			echo do_shortcode('[porto_block id="364704"]');
		} elseif(is_product_category('wooden-toys')) {
			echo do_shortcode('[porto_block id="364997"]');
		} else {
			echo do_shortcode('[porto_block id="363351"]');
		}
